...
├── haproxy/
│   ├── setup-haproxy.sh                 # haproxy + TLS + bunny edge refresher; emits /etc/haproxy/conf.d/*.cfg
│   ├── setup-haproxy-remote.sh          # LOCAL wrapper (full setup)
│   └── deploy-haproxy-config-remote.sh  # LOCAL wrapper — config-only, hitless reload (no cert/pkg churn)
└── srs/
...
> `deploy-haproxy-config-remote.sh` — it regenerates `conf.d/*.cfg` and does a
> hitless reload without touching packages, certs, sysctl, or UFW. See
...
# Fast HAProxy config-only deploy — regenerate /etc/haproxy/conf.d/*.cfg from
# .env + hitless reload (keeps live publishers up). Skips packages, TLS cert
# issuance, the bunny refresher, sysctl, and UFW. Use for routing / rate-limit
# tweaks (e.g. the per-IP RTMP/RTMPS conn cap) on an already-provisioned edge.
